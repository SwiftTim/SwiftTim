# 👋 Hi, I'm Timothy Muthama (SwiftTim)!

<p align="center">
  <img src="header.png?v=2" alt="Timothy Muthama Banner" width="100%">
</p>

## 🚀 Full-stack Developer | Cybersecurity Enthusiast | UI/UX Designer
I build high-performance, secure applications with a focus on clean code and intuitive user experiences. My expertise ranges from building complex ERP systems to developing advanced encrypted traffic pattern analyzers.

---

### 🛠️ Tech Stack & Tools
- **Languages:** PHP, JavaScript, TypeScript, Python, C++, Dart
- **Frontend:** React, Blade, Flutter, HTML5, CSS3, TailwindCSS
- **Backend:** Node.js, PHP/Laravel, Firebase, MySQL, PostgreSQL
- **Security:** Traffic Analysis, SSL Tunneling, E2E Encryption
- **Tools:** Git/GitHub, VS Code, Figma, Postman

---

### 📁 Featured Projects
#### 📊 [School Management ERP](https://github.com/SwiftTim/erp)
A complete ERP featuring a **Constraint-Based Timetable Engine**. Uses intelligent backtracking algorithms to handle complex scheduling demands. 
*Tech: PHP, MySQL, JavaScript*

#### 🔐 [Nexus Encrypted Chat](https://github.com/SwiftTim/Nexus)
An end-to-end encrypted chat gateway designed for absolute privacy using SSL Tunneling to bypass ISP filters.
*Tech: TypeScript, Security Architectures*

#### 🛰️ [ETPA Analyzer](https://github.com/SwiftTim/Encrypted-Traffic-Pattern-Analyzer-ETPA)
Encrypted Traffic Pattern Analyzer for cybersecurity. Analyzes metadata patterns in E2E traffic without breaking encryption.
*Tech: Python, Cybersecurity Tools*

#### 🎁 [Donations Hub](https://github.com/SwiftTim/Donations)
A charity management platform with dynamic campaign tracking and integrated payment processing.
*Tech: Full-stack Development*

---

### 📜 Certifications
- **PMI CAPM®** (Project Management)
- **ITIL® 4 Foundation** (Service Management)
- **IBM** Introduction to Cybersecurity Tools
- **Fundamentals of AI & ML**

---

### 📊 GitHub Stats
<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=SwiftTim&theme=tokyonight&margin-w=15" alt="GitHub Trophies" />
  <br>
  <img src="https://streak-stats.demolab.com/?user=SwiftTim&theme=tokyonight" alt="GitHub Streak" />
</p>

---

### 📫 Let's Connect!
- 🌐 [My Portfolio](https://swifttim.github.io/portfolio/home-one.html#)
- 💼 [LinkedIn](YOUR_LINKEDIN_URL_HERE)
- 📧 [Email Me](mailto:YOUR_EMAIL_HERE)
- 🆔 [ORCID Profile](https://orcid.org/0009-0000-0344-3665)
